...
parser.add_argument("-Region", help="Workspace ONE Intelligence API region. More information here: https://docs.vmware.com/en/VMware-Workspace-ONE/services/intelligence-documentation/GUID-04_intel_reqs.html", required=True)
...
def run_report(report_id, url, access_token):
    try:
        ...
        time.sleep(60) # add delays to prevent race condition where reports are not yet generated but report list may be retrieved
    except requests.exceptions.RequestException as e:
